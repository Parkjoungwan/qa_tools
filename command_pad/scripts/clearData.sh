...
for SERIAL in "${TARGET_SERIALS[@]}"; do
    ...
    if $ADB shell am start -n "$ACTIVITY" >/dev/null 2>&1; then
      echo "✅ Success (am start)"
    elif $ADB shell monkey --pct-syskeys 0 -p "$PKG" -c android.intent.category.LAUNCHER 1 >/dev/null 2>&1; then
      echo "✅ Success (monkey fallback)"
    else
      echo "❌ Failed to restart app on $SERIAL"
    fi
done